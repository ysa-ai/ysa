...
| 3. Container images | Checks `sandbox-claude`, `sandbox-mistral`, `sandbox-proxy` exist — prints `bun run build:images` if any are missing |
...
- If setup reports missing images, build them with `bun run build:images` from the repo root, then re-run `ysa setup`.
